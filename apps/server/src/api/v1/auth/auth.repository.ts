...
import { UserCreateData, UserLoginData } from './auth.types.js'; // Added .js extension

export class AuthRepository {
    private usersCollection = 'users'; // Or your specific PocketBase collection name
     ...
     */
    async authenticateUser(loginData: UserLoginData) {
        try {
            const authData = await pb.collection(this.usersCollection).authWithPassword(
                loginData.email,
                loginData.password,
            );
    ...
    // Add other methods as needed, e.g., findUserById, updateUser, etc.
}
