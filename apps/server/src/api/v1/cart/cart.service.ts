...
export class CartService {
   ...
   */
  async getOrCreateCartForUserOrSession(
    ...
    if (!sessionId) {
      ...
      throw new AppError(
        "Session ID is required to get or create a cart.",
        400
      );
    }
    ...
    if (userId) {
      ...
      if (guestCart) {
        if (cart) {
          ...
          console.log(
            `Merging guest cart ${guestCart.id} into user cart ${cart.id}`
          );
          ...
          console.log(
            `Transferring guest cart ${guestCart.id} to user ${userId}`
          );
  ...
  async removeItemFromCart(
    cartId: string,
    itemInput: RemoveItemFromCartInput
  ): Promise<Cart> {
    const cart = await this.cartRepository.findById(cartId);
    if (!cart) {
      throw new NotFoundError("Cart not found.");
    ...
    const updatedItems = removeCartItem(
      cart.items || [],
      itemInput.productId,
      itemInput.variantId
    );
    ...
    if (updatedItems.length === (cart.items || []).length) {
      ...
      console.warn(
        `Attempted to remove item (product: ${itemInput.productId}, variant: ${itemInput.variantId}) not found in cart ${cartId}`
      );
  ...
  async updateCartItem(
    ...
    if (newQuantity !== undefined) {
      updatedItems = updateCartItemQuantity(
        cart.items || [],
  ...
  async applyCoupon(cartId: string, couponCode: string): Promise<Cart> {
    ...
    if (!finalUpdatedCart) {
      throw new AppError(
        "Failed to recalculate totals after applying coupon.",
        500
      );
    }
  ...
  async removeCoupon(cartId: string): Promise<Cart> {
    ...
    if (!finalUpdatedCart) {
      throw new AppError(
        "Failed to recalculate totals after removing coupon.",
        500
      );
    }
  ...
  async updateCartDetails(
    cartId: string,
    detailsInput: UpdateCartDetailsInput
  ): Promise<Cart> {
    const cart = await this.cartRepository.findById(cartId);
    if (!cart) {
      throw new NotFoundError("Cart not found.");
    ...
    const updatedCartWithDetails = await this.cartRepository.update(
      cartId,
      updateData
    );
    ...
    if (!finalUpdatedCart) {
      throw new AppError(
        "Failed to recalculate totals after updating details.",
        500
      );
    }
  ...
  async transferCartToUser(input: TransferCartInput): Promise<Cart> {
    ...
    if (!guestCart) {
      ...
      console.warn(
        `No guest cart found for session ${input.sessionId} to transfer to user ${input.userId}.`
      );
      return this.getOrCreateCartForUserOrSession(
        input.userId,
        input.sessionId
      ); // Fallback to standard user cart logic
    }
    ...
    if (userCart) {
      ...
      console.log(
        `Merging guest cart ${guestCart.id} into existing user cart ${userCart.id}.`
      );
      ...
      console.log(
        `Assigning guest cart ${guestCart.id} to user ${input.userId}.`
      );
